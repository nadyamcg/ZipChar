...
cmd.add_argument("-d", "--directory", help="What sub-directory to filter for?", required=False)
cmd.add_argument("-p", "--prefix", help="What file prefix to filter for?", required=False)
...
if not args.file.endswith(('.zip', '.kpf')):
    print("Input file must be .zip or .kpf!", file=sys.stderr)
    sys.exit(1)
...
# you should avoid redefining `zip`, since `zip()` is built-in
with zipfile.ZipFile(args.file, 'r') as zf:
    for path in zf.namelist():
        ...
        if args.directory and not dir_part.startswith(args.directory):
            continue

        # match file name with desired prefix
        if args.prefix and not file_part.startswith(args.prefix):
            continue
